...
using JosephGuadagno.Broadcasting.Functions.Dispatchers;
using JosephGuadagno.Broadcasting.Functions.Services;
using Microsoft.Extensions.Logging.Abstractions;
using Moq;

namespace JosephGuadagno.Broadcasting.Functions.Tests.Dispatchers;

public class ScheduledItemsTests
{
    private readonly Mock<IScheduledItemManager> _scheduledItemManager;
    private readonly Mock<IScheduledItemEventDispatcher> _scheduledItemEventDispatcher;
    ...
    public ScheduledItemsTests()
    {
        ...
        _scheduledItemEventDispatcher = new Mock<IScheduledItemEventDispatcher>();
    ...
    public async Task RunAsync_WhenScheduledItemsExist_PublishesMarksSentAndUpdatesFeedCheck()
    {
        ...
        _feedCheckManager
            .Setup(m => m.GetByNameAsync(ConfigurationFunctionNames.DispatchersScheduledItems, scheduledItem.CreatedByEntraOid!, It.IsAny<CancellationToken>()))
        ...
        _feedCheckManager.Verify(m => m.SaveAsync(
            It.Is<FeedCheck>(f => f.Name == ConfigurationFunctionNames.DispatchersScheduledItems && f.EntraOId == scheduledItem.CreatedByEntraOid),
            It.IsAny<CancellationToken>()), Times.Once);
    }
    ...
    public async Task RunAsync_WhenOneScheduledItemPublishFails_ContinuesProcessingRemainingItems()
    {
        ...
        _feedCheckManager
            .Setup(m => m.GetByNameAsync(ConfigurationFunctionNames.DispatchersScheduledItems, failedItem.CreatedByEntraOid!, It.IsAny<CancellationToken>()))
            .ReturnsAsync(new FeedCheck
            {
                Id = 1,
                Name = ConfigurationFunctionNames.DispatchersScheduledItems,
