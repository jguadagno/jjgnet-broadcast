...
public class CollectorEventPublisherTests
{
    private const string OwnerOid = "test-owner-oid";

    private readonly Mock<IUserEventPublisherMappingDataStore> _mappingDataStore;
    ...
    private readonly CollectorEventPublisher _sut;

    public CollectorEventPublisherTests()
    {
        _mappingDataStore = new Mock<IUserEventPublisherMappingDataStore>();
        ...
        _sut = new CollectorEventPublisher(
            _mappingDataStore.Object,
            _messageTemplateManager.Object,
            _postComposer.Object,
            _queueServiceClient.Object,
            NullLogger<CollectorEventPublisher>.Instance);
    }

    private static UserEventPublisherMapping ActiveMapping(int platformId) => new()
    {
        Id = 1,
    ...
    // --- PublishSyndicationFeedItemAsync ---

    [Fact]
    public async Task PublishSyndicationFeedItemAsync_WhenNoMappingsFound_DoesNotSendToQueue()
    {
        _mappingDataStore
            ...
            .ReturnsAsync(new List<UserEventPublisherMapping>());

        await _sut.PublishSyndicationFeedItemAsync(BuildFeedItem(), OwnerOid);

        _queueServiceClient.Verify(q => q.GetQueueClient(It.IsAny<string>()), Times.Never);
    }

    [Fact]
    public async Task PublishSyndicationFeedItemAsync_WhenMappingAndTemplateFound_SendsToCorrectQueue()
    {
        var mapping = ActiveMapping(SocialMediaPlatformIds.Twitter);
        _mappingDataStore
            .Setup(s => s.GetByUserAndEventTypeAsync(OwnerOid, MessageTemplates.MessageTypes.NewSyndicationFeedItem, It.IsAny<CancellationToken>()))
            .ReturnsAsync(new List<UserEventPublisherMapping> { mapping });
    ...
    [Fact]
    public async Task PublishSyndicationFeedItemAsync_WhenTemplateNotFound_DoesNotSendToQueue()
    {
    ...
    // --- PublishYouTubeItemAsync ---

    [Fact]
    public async Task PublishYouTubeItemAsync_WhenNoMappingsFound_DoesNotSendToQueue()
    {
        _mappingDataStore
            .Setup(s => s.GetByUserAndEventTypeAsync(OwnerOid, MessageTemplates.MessageTypes.NewYouTubeItem, It.IsAny<CancellationToken>()))
            .ReturnsAsync(new List<UserEventPublisherMapping>());

        await _sut.PublishYouTubeItemAsync(BuildYouTubeItem(), OwnerOid);

        _queueServiceClient.Verify(q => q.GetQueueClient(It.IsAny<string>()), Times.Never);
    }

    [Fact]
    public async Task PublishYouTubeItemAsync_WhenMappingAndTemplateFound_SendsToCorrectQueue()
    {
    ...
    [Fact]
    public async Task PublishYouTubeItemAsync_WhenTemplateNotFound_DoesNotSendToQueue()
    {
        var mapping = new UserEventPublisherMapping
        {
            Id = 2,
    ...
    // --- PublishSpeakingEngagementAsync ---

    [Fact]
    public async Task PublishSpeakingEngagementAsync_WhenNoMappingsFound_DoesNotSendToQueue()
    {
        _mappingDataStore
        ...
        await _sut.PublishSpeakingEngagementAsync(BuildEngagement(), OwnerOid);

        _queueServiceClient.Verify(q => q.GetQueueClient(It.IsAny<string>()), Times.Never);
    }

    [Fact]
    public async Task PublishSpeakingEngagementAsync_WhenMappingAndTemplateFound_SendsToCorrectQueue()
    {
        var mapping = new UserEventPublisherMapping
        {
            Id = 3,
    ...
    [Fact]
    public async Task PublishSpeakingEngagementAsync_WhenTemplateNotFound_DoesNotSendToQueue()
    {
        var mapping = new UserEventPublisherMapping
        {
            Id = 3,
    ...
    [Fact]
    public async Task PublishSyndicationFeedItemAsync_WhenMultipleMappings_SendsToAllConfiguredQueues()
    {
        var twitterMapping = ActiveMapping(SocialMediaPlatformIds.Twitter);
        var blueskyMapping = new UserEventPublisherMapping
        {
        ...
        _mappingDataStore
            .Setup(s => s.GetByUserAndEventTypeAsync(OwnerOid, MessageTemplates.MessageTypes.NewSyndicationFeedItem, It.IsAny<CancellationToken>()))
            .ReturnsAsync(new List<UserEventPublisherMapping> { twitterMapping, blueskyMapping });
