...
public class ScheduledItemEventPublisherTests
{
    ...
    private readonly ScheduledItemEventPublisher _sut;

    public ScheduledItemEventPublisherTests()
    {
        ...
        _sut = new ScheduledItemEventPublisher(
            ...
            NullLogger<ScheduledItemEventPublisher>.Instance);
    }
    ...
    [Fact]
    public async Task PublishAsync_WhenNoMappingsFound_DoesNotSendToQueue()
    {
        _mappingDataStore
        ...
        await _sut.PublishAsync(BuildScheduledItem());

        _queueServiceClient.Verify(q => q.GetQueueClient(It.IsAny<string>()), Times.Never);
    }

    [Fact]
    public async Task PublishAsync_WhenFeedItemMappingAndTemplateFound_SendsToTwitterQueue()
    {
        var scheduledItem = BuildScheduledItem(ScheduledItemType.SyndicationFeedItems);
        _mappingDataStore
            .Setup(s => s.GetByUserAndEventTypeAsync(OwnerOid, MessageTemplates.MessageTypes.ScheduledItem, It.IsAny<CancellationToken>()))
            .ReturnsAsync(new List<UserEventPublisherMapping> { ActiveMapping(SocialMediaPlatformIds.Twitter) });
        ...
        await _sut.PublishAsync(scheduledItem);
    ...
    [Fact]
    public async Task PublishAsync_WhenYouTubeItemMappingAndTemplateFound_SendsToLinkedInQueue()
    {
        var scheduledItem = BuildScheduledItem(ScheduledItemType.YouTubeItems);
        _mappingDataStore
            .Setup(s => s.GetByUserAndEventTypeAsync(OwnerOid, MessageTemplates.MessageTypes.ScheduledItem, It.IsAny<CancellationToken>()))
            .ReturnsAsync(new List<UserEventPublisherMapping> { ActiveMapping(SocialMediaPlatformIds.LinkedIn) });
        _youTubeItemManager
            .Setup(m => m.GetAsync(42, It.IsAny<CancellationToken>()))
    ...
    [Fact]
    public async Task PublishAsync_WhenEngagementItemMappingAndTemplateFound_SendsToBlueskyQueue()
    {
        var scheduledItem = BuildScheduledItem(ScheduledItemType.Engagements);
        _mappingDataStore
            .Setup(s => s.GetByUserAndEventTypeAsync(OwnerOid, MessageTemplates.MessageTypes.ScheduledItem, It.IsAny<CancellationToken>()))
            .ReturnsAsync(new List<UserEventPublisherMapping> { ActiveMapping(SocialMediaPlatformIds.Bluesky) });
        _engagementManager
            .Setup(m => m.GetAsync(42, It.IsAny<CancellationToken>()))
    ...
    [Fact]
    public async Task PublishAsync_WhenTalkItemMappingAndTemplateFound_SendsToFacebookQueue()
    {
        var scheduledItem = BuildScheduledItem(ScheduledItemType.Talks);
        _mappingDataStore
            .Setup(s => s.GetByUserAndEventTypeAsync(OwnerOid, MessageTemplates.MessageTypes.ScheduledItem, It.IsAny<CancellationToken>()))
            .ReturnsAsync(new List<UserEventPublisherMapping> { ActiveMapping(SocialMediaPlatformIds.Facebook) });
        _engagementManager
    ...
    [Fact]
    public async Task PublishAsync_WhenTemplateNotFound_DoesNotSendToQueue()
    {
        _mappingDataStore
            .Setup(s => s.GetByUserAndEventTypeAsync(OwnerOid, MessageTemplates.MessageTypes.ScheduledItem, It.IsAny<CancellationToken>()))
            .ReturnsAsync(new List<UserEventPublisherMapping> { ActiveMapping(SocialMediaPlatformIds.Twitter) });
