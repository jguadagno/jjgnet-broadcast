...
/// <summary>Shows the aggregate platform settings summary for the current user.</summary>
[Authorize(Policy = AuthorizationPolicyNames.RequireContributor)]
[Route("Platforms")]
public class PlatformsController(
    IPlatformsAggregateService aggregateService,
    ISocialMediaPlatformService platformService) : Controller
{
    ...
    public async Task<IActionResult> Index()
    {
        ...
        var viewModel = aggregate ?? new PlatformsAggregateViewModel();
        viewModel.Platforms = cards;

        return View(viewModel);
    }

    private static bool IsConfigured(PlatformsAggregateViewModel? aggregate, string platformName) =>
        platformName switch
        {
    ...
    private static bool IsEnabled(PlatformsAggregateViewModel? aggregate, string platformName) =>
        platformName switch
        {
