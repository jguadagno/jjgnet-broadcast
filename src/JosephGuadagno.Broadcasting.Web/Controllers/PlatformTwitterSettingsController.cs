...
/// <summary>Manages Twitter/X platform settings for the current user.</summary>
[Authorize(Policy = AuthorizationPolicyNames.RequireContributor)]
[Route("Platforms/Twitter/Settings")]
public class PlatformTwitterSettingsController(
    IUserPlatformTwitterSettingsService service,
    ISetupService setupService,
    ILogger<PlatformTwitterSettingsController> logger) : Controller
{
    [HttpGet("")]
    [HttpGet("Index")]
    ...
    public async Task<IActionResult> Save(TwitterPlatformSettingsViewModel model)
    {
        ...
        var settings = new UserPlatformTwitterSettings
        {
            Id = model.Id,
    ...
    private static TwitterPlatformSettingsViewModel MapToViewModel(UserPlatformTwitterSettings? settings) =>
        new()
        {
            Id = settings?.Id ?? 0,
