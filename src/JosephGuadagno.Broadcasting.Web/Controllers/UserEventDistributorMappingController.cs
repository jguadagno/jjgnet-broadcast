...
public class UserEventDistributorMappingController(
    IUserEventDistributorMappingService mappingService,
    ...
    ISetupService setupService) : Controller
{
    ...
    [Authorize(Policy = AuthorizationPolicyNames.RequireContributor)]
    public async Task<IActionResult> Create(UserEventDistributorMappingViewModel viewModel)
    {
    ...
    [ValidateAntiForgeryToken]
    [Authorize(Policy = AuthorizationPolicyNames.RequireContributor)]
    public async Task<IActionResult> Edit(int id, UserEventDistributorMappingViewModel viewModel)
    {
        if (id != viewModel.Id)
        {
            return NotFound();
    ...
    private async Task<UserEventDistributorMappingViewModel> BuildViewModelAsync(UserEventDistributorMapping mapping)
    {
        var platforms = await socialMediaPlatformService.GetAllAsync(pageSize: 100);
        var platformLookup = platforms.Items.ToDictionary(platform => platform.Id);
    ...
    private async Task PopulateOptionsAsync(UserEventDistributorMappingViewModel viewModel)
    {
        var platforms = await socialMediaPlatformService.GetAllAsync(pageSize: 100);
        viewModel.SocialMediaPlatforms = BuildPlatformOptions(platforms.Items, viewModel.SocialMediaPlatformId);
        viewModel.EventTypes = BuildEventTypeOptions(viewModel.EventType);
    ...
    private static List<SelectListItem> BuildEventTypeOptions(string? selectedEventType)
    {
        ...
        [
            ...
            .. DistributorEventTypes.All.Select(option => new SelectListItem(option.Label, option.Value)
            {
    ...
    private static UserEventDistributorMappingViewModel MapToViewModel(
        ...
        IReadOnlyDictionary<int, SocialMediaPlatform> platformLookup)
    {
        ...
        var eventType = DistributorEventTypes.Get(mapping.EventType);

        return new UserEventDistributorMappingViewModel
        {
            Id = mapping.Id,
    ...
    private static UserEventDistributorMapping MapToDomainModel(UserEventDistributorMappingViewModel viewModel)
    {
        return new UserEventDistributorMapping
        {
            Id = viewModel.Id,
