...
public class YouTubeItemsController : Controller
{
    ...
    public async Task<IActionResult> Index(int page = Pagination.DefaultPage, string sortBy = "name", bool sortDescending = false, string? filter = null)
    {
        ...
        ViewBag.ControllerName = "YouTubeItems";
    ...
    public async Task<IActionResult> Details(int id)
    {
        ...
        var sourceViewModel = _mapper.Map<YouTubeItemViewModel>(source);
    ...
    {
        return View(new YouTubeItemViewModel { PublicationDate = DateTimeOffset.UtcNow });
    }

    /// <summary>
    /// Adds a new YouTube source
    /// </summary>
    /// <param name="sourceViewModel">The <see cref="YouTubeItemViewModel"/></param>
    /// <returns>Upon success, redirects to the <see cref="Details"/> page. Upon failure, redirects to the <see cref="Add()"/> page.</returns>
    [HttpPost]
    [ValidateAntiForgeryToken]
    [Authorize(Policy = AuthorizationPolicyNames.RequireContributor)]
    public async Task<IActionResult> Add(YouTubeItemViewModel sourceViewModel)
    {
