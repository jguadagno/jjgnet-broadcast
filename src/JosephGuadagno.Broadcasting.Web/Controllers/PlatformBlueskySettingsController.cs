...
/// <summary>Manages Bluesky platform settings for the current user.</summary>
[Authorize(Policy = AuthorizationPolicyNames.RequireContributor)]
[Route("Platforms/Bluesky/Settings")]
public class PlatformBlueskySettingsController(
    IUserPlatformBlueskySettingsService service,
    ISetupService setupService,
    ILogger<PlatformBlueskySettingsController> logger) : Controller
{
    [HttpGet("")]
    [HttpGet("Index")]
    ...
    public async Task<IActionResult> Save(BlueskyPlatformSettingsViewModel model)
    {
        ...
        var settings = new UserPlatformBlueskySettings
        {
            Id = model.Id,
    ...
    private static BlueskyPlatformSettingsViewModel MapToViewModel(UserPlatformBlueskySettings? settings) =>
        new()
        {
            Id = settings?.Id ?? 0,
