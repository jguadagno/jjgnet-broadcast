...
public class UserEventDispatcherMappingServiceTests
{
    private readonly Mock<IDownstreamApi> _apiClient = new();
    private readonly Mock<ILogger<UserEventDispatcherMappingService>> _logger = new();
    ...
    public async Task AddAsync_ShouldSendExpectedApiContract()
    {
        ...
        _apiClient
            .Setup(api => api.PostForUserAsync<object, UserEventDispatcherMapping>(
                It.IsAny<string>(),
                It.IsAny<object>(),
            ...
            .ReturnsAsync(new UserEventDispatcherMapping { Id = 15, EventType = mapping.EventType, SocialMediaPlatformId = mapping.SocialMediaPlatformId });

        var sut = new UserEventDispatcherMappingService(_apiClient.Object, _logger.Object);

        var result = await sut.AddAsync(mapping);

        capturedOptions.Should().NotBeNull();
        capturedOptions!.RelativePath.Should().Be("/Dispatchers/EventDispatcherMappings");
