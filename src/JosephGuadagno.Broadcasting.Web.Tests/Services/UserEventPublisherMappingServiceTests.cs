...
public class UserEventPublisherMappingServiceTests
{
    private readonly Mock<IDownstreamApi> _apiClient = new();
    private readonly Mock<ILogger<UserEventPublisherMappingService>> _logger = new();
    ...
    public async Task AddAsync_ShouldSendExpectedApiContract()
    {
        ...
        _apiClient
            .Setup(api => api.PostForUserAsync<object, UserEventPublisherMapping>(
                It.IsAny<string>(),
                It.IsAny<object>(),
            ...
            .ReturnsAsync(new UserEventPublisherMapping { Id = 15, EventType = mapping.EventType, SocialMediaPlatformId = mapping.SocialMediaPlatformId });

        var sut = new UserEventPublisherMappingService(_apiClient.Object, _logger.Object);

        var result = await sut.AddAsync(mapping);

        capturedOptions.Should().NotBeNull();
        capturedOptions!.RelativePath.Should().Be("/Publishers/EventPublisherMappings");
