...
/// <summary>
/// Unit tests for <see cref="SyndicationFeedItemsController"/>.
///
...
{
    private readonly Mock<ISyndicationFeedItemService> _service;
    private readonly Mock<IMapper> _mapper;
    private readonly SyndicationFeedItemsController _controller;

    public SyndicationFeedItemsControllerTests()
    {
        _service = new Mock<ISyndicationFeedItemService>();
        _mapper = new Mock<IMapper>();
        _controller = new SyndicationFeedItemsController(_service.Object, _mapper.Object);
    ...
    private static SyndicationFeedItem BuildSource(int id, string oid = "owner-oid-12345") =>
    ...
    [Fact]
    public async Task Index_ShouldReturnViewWithSyndicationFeedItemViewModels()
    {
        // Arrange
        var sources = new List<SyndicationFeedItem> { BuildSource(1) };
        var viewModels = new List<SyndicationFeedItemViewModel> { new SyndicationFeedItemViewModel { Id = 1 } };
        ...
        _mapper.Setup(m => m.Map<List<SyndicationFeedItemViewModel>>(It.IsAny<object>())).Returns(viewModels);
    ...
    public async Task Details_WhenSourceFound_AndUserIsOwner_ShouldReturnView()
    {
        ...
        var viewModel = new SyndicationFeedItemViewModel { Id = 1 };
        SetOwnerUser(oid);
        _service.Setup(s => s.GetAsync(1)).ReturnsAsync(source);
        _mapper.Setup(m => m.Map<SyndicationFeedItemViewModel>(It.IsAny<object>())).Returns(viewModel);
    ...
    {
        // Arrange
        _service.Setup(s => s.GetAsync(99)).ReturnsAsync((SyndicationFeedItem?)null);
    ...
    public async Task Details_WhenUserIsNotOwnerAndNotAdmin_ShouldRedirectWithError()
    {
        ...
        _mapper.Verify(m => m.Map<SyndicationFeedItemViewModel>(It.IsAny<object>()), Times.Never);
    }
    ...
    [Fact]
    public void Add_Get_ShouldReturnViewWithNewSyndicationFeedItemViewModel()
    {
        // Act
        ...
        // Assert
        var viewResult = Assert.IsType<ViewResult>(result);
        Assert.IsType<SyndicationFeedItemViewModel>(viewResult.Model);
    }
    ...
    public async Task Add_Post_WhenModelStateIsInvalid_ShouldReturnView()
    {
        ...
        var viewModel = new SyndicationFeedItemViewModel();
        ...
        var viewResult = Assert.IsType<ViewResult>(result);
        Assert.Equal(viewModel, viewResult.Model);
        _service.Verify(s => s.SaveAsync(It.IsAny<SyndicationFeedItem>()), Times.Never);
    }
    ...
    {
        // Arrange
        var viewModel = new SyndicationFeedItemViewModel { Id = 0 };
        var savedSource = BuildSource(42);
        SetOwnerUser();
        _mapper.Setup(m => m.Map<SyndicationFeedItem>(It.IsAny<object>())).Returns(BuildSource(0));
        _service.Setup(s => s.SaveAsync(It.IsAny<SyndicationFeedItem>())).ReturnsAsync(savedSource);
    ...
    public async Task Add_Post_WhenSaveFails_ShouldRedirectToAdd()
    {
        ...
        SetOwnerUser();
        _mapper.Setup(m => m.Map<SyndicationFeedItem>(It.IsAny<object>())).Returns(BuildSource(0));
        _service.Setup(s => s.SaveAsync(It.IsAny<SyndicationFeedItem>())).ReturnsAsync((SyndicationFeedItem?)null);
