...
{
    private readonly Mock<IUserEventDistributorMappingService> _mappingService = new();
    ...
    private readonly UserEventDistributorMappingController _controller;

    public UserEventDistributorMappingControllerTests()
    {
        _controller = new UserEventDistributorMappingController(_mappingService.Object, _platformService.Object, _setupService.Object)
        {
    ...
    public async Task Index_ShouldReturnViewWithDisplayMetadata()
    {
        ...
        [
            new UserEventDistributorMapping
            {
                Id = 1,
        ...
        var model = Assert.IsAssignableFrom<List<UserEventDistributorMappingViewModel>>(viewResult.Model);
    ...
    public async Task Create_Post_WhenValid_ShouldRedirectToIndexAndInvalidateSetup()
    {
        ...
        _mappingService.Setup(service => service.AddAsync(It.IsAny<UserEventDistributorMapping>()))
            .Callback<UserEventDistributorMapping>(mapping => captured = mapping)
            .ReturnsAsync(new UserEventDistributorMapping { Id = 9 });

        var viewModel = new UserEventDistributorMappingViewModel
        {
        ...
        redirectResult.ActionName.Should().Be(nameof(UserEventDistributorMappingController.Index));
    ...
    public async Task Create_Post_WhenModelStateInvalid_ShouldRepopulateOptions()
    {
        ...
        var result = await _controller.Create(new UserEventDistributorMappingViewModel());

        var viewResult = Assert.IsType<ViewResult>(result);
        var model = Assert.IsType<UserEventDistributorMappingViewModel>(viewResult.Model);
        model.EventTypes.Should().NotBeEmpty();
        model.SocialMediaPlatforms.Should().NotBeEmpty();
        _mappingService.Verify(service => service.AddAsync(It.IsAny<UserEventDistributorMapping>()), Times.Never);
    }
}
