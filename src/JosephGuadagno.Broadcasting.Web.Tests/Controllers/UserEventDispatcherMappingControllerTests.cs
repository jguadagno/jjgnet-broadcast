...
{
    private readonly Mock<IUserEventDispatcherMappingService> _mappingService = new();
    ...
    private readonly UserEventDispatcherMappingController _controller;

    public UserEventDispatcherMappingControllerTests()
    {
        _controller = new UserEventDispatcherMappingController(_mappingService.Object, _platformService.Object, _setupService.Object)
        {
    ...
    public async Task Index_ShouldReturnViewWithDisplayMetadata()
    {
        ...
        [
            new UserEventDispatcherMapping
            {
                Id = 1,
        ...
        var model = Assert.IsAssignableFrom<List<UserEventDispatcherMappingViewModel>>(viewResult.Model);
    ...
    public async Task Create_Post_WhenValid_ShouldRedirectToIndexAndInvalidateSetup()
    {
        ...
        _mappingService.Setup(service => service.AddAsync(It.IsAny<UserEventDispatcherMapping>()))
            .Callback<UserEventDispatcherMapping>(mapping => captured = mapping)
            .ReturnsAsync(new UserEventDispatcherMapping { Id = 9 });
        ...
        redirectResult.ActionName.Should().Be(nameof(UserEventDispatcherMappingController.Index));
    ...
    public async Task Create_Post_WhenModelStateInvalid_ShouldRepopulateOptions()
    {
        ...
        var result = await _controller.Create(new UserEventDispatcherMappingViewModel());

        var viewResult = Assert.IsType<ViewResult>(result);
        var model = Assert.IsType<UserEventDispatcherMappingViewModel>(viewResult.Model);
        model.EventTypes.Should().NotBeEmpty();
        model.SocialMediaPlatforms.Should().NotBeEmpty();
        _mappingService.Verify(service => service.AddAsync(It.IsAny<UserEventDispatcherMapping>()), Times.Never);
    }
}
