...
/// <summary>
/// Unit tests for <see cref="YouTubeItemsController"/>.
///
...
{
    private readonly Mock<IYouTubeItemService> _service;
    private readonly Mock<IMapper> _mapper;
    private readonly YouTubeItemsController _controller;

    public YouTubeItemsControllerTests()
    {
        _service = new Mock<IYouTubeItemService>();
        _mapper = new Mock<IMapper>();
        _controller = new YouTubeItemsController(_service.Object, _mapper.Object);
    ...
    private static YouTubeItem BuildSource(int id, string oid = "owner-oid-12345") =>
        new YouTubeItem
        {
            Id = id,
    ...
    [Fact]
    public async Task Index_ShouldReturnViewWithYouTubeItemViewModels()
    {
        // Arrange
        var sources = new List<YouTubeItem> { BuildSource(1) };
        var viewModels = new List<YouTubeItemViewModel> { new YouTubeItemViewModel { Id = 1 } };
        ...
        _mapper.Setup(m => m.Map<List<YouTubeItemViewModel>>(It.IsAny<object>())).Returns(viewModels);
    ...
    public async Task Details_WhenSourceFound_AndUserIsOwner_ShouldReturnView()
    {
        ...
        var viewModel = new YouTubeItemViewModel { Id = 1 };
        SetOwnerUser(oid);
        _service.Setup(s => s.GetAsync(1)).ReturnsAsync(source);
        _mapper.Setup(m => m.Map<YouTubeItemViewModel>(It.IsAny<object>())).Returns(viewModel);
    ...
    {
        // Arrange
        _service.Setup(s => s.GetAsync(99)).ReturnsAsync((YouTubeItem?)null);
    ...
    public async Task Details_WhenUserIsNotOwnerAndNotAdmin_ShouldRedirectWithError()
    {
        ...
        _mapper.Verify(m => m.Map<YouTubeItemViewModel>(It.IsAny<object>()), Times.Never);
    }
    ...
    [Fact]
    public void Add_Get_ShouldReturnViewWithNewYouTubeItemViewModel()
    {
        // Act
        ...
        // Assert
        var viewResult = Assert.IsType<ViewResult>(result);
        Assert.IsType<YouTubeItemViewModel>(viewResult.Model);
    }
    ...
    public async Task Add_Post_WhenModelStateIsInvalid_ShouldReturnView()
    {
        ...
        var viewModel = new YouTubeItemViewModel();
        ...
        var viewResult = Assert.IsType<ViewResult>(result);
        Assert.Equal(viewModel, viewResult.Model);
        _service.Verify(s => s.SaveAsync(It.IsAny<YouTubeItem>()), Times.Never);
    }
    ...
    {
        // Arrange
        var viewModel = new YouTubeItemViewModel { Id = 0 };
        var savedSource = BuildSource(42);
        SetOwnerUser();
        _mapper.Setup(m => m.Map<YouTubeItem>(It.IsAny<object>())).Returns(BuildSource(0));
        _service.Setup(s => s.SaveAsync(It.IsAny<YouTubeItem>())).ReturnsAsync(savedSource);
    ...
    public async Task Add_Post_WhenSaveFails_ShouldRedirectToAdd()
    {
        ...
        SetOwnerUser();
        _mapper.Setup(m => m.Map<YouTubeItem>(It.IsAny<object>())).Returns(BuildSource(0));
        _service.Setup(s => s.SaveAsync(It.IsAny<YouTubeItem>())).ReturnsAsync((YouTubeItem?)null);
