...
/// <summary>
/// Tests for UserEventDispatcherMappingDataStore.
/// </summary>
public class UserEventDispatcherMappingDataStoreTests : IDisposable
{
    private readonly BroadcastingContext _context;
    private readonly Mock<ILogger<UserEventDispatcherMappingDataStore>> _logger = new();
    private readonly UserEventDispatcherMappingDataStore _dataStore;

    public UserEventDispatcherMappingDataStoreTests()
    {
    ...
    private async Task CreateMappingAsync(
        ...
        bool isActive = true)
    {
        _context.UserEventDispatcherMappings.Add(new UserEventDispatcherMapping
        {
    ...
    public async Task GetByIdAsync_ReturnsMappingById()
    {
        ...
        var entity = await _context.UserEventDispatcherMappings.FirstAsync(s => s.CreatedByEntraOid == ownerOid);
    ...
    {
        const string ownerOid = "user-a-oid-11111111";
        var mapping = new Domain.Models.UserEventDispatcherMapping
        {
            CreatedByEntraOid = ownerOid,
        ...
        var persisted = await _context.UserEventDispatcherMappings.FirstAsync(m => m.Id == result.Id);
    ...
    public async Task SaveAsync_UpdatesExistingMappingById()
    {
        ...
        var existing = await _context.UserEventDispatcherMappings.FirstAsync(m => m.CreatedByEntraOid == ownerOid);
        ...
        var count = await _context.UserEventDispatcherMappings.CountAsync(m => m.CreatedByEntraOid == ownerOid);
    ...
    public async Task DeleteAsync_ReturnsFalseWhenOwnerMismatch()
    {
        ...
        const string ownerBOid = "user-b-oid-22222222";
        await CreateMappingAsync(ownerAOid, Domain.Constants.MessageTemplates.MessageTypes.NewSyndicationFeedItem, 1);
        var entity = await _context.UserEventDispatcherMappings.FirstAsync(m => m.CreatedByEntraOid == ownerAOid);
