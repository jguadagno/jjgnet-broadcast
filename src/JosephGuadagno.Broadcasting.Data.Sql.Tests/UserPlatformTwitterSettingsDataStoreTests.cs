...
/// <summary>
/// Tests for UserPlatformTwitterSettingsDataStore — isolation and owner enforcement
/// </summary>
public class UserPlatformTwitterSettingsDataStoreTests : IDisposable
{
    private readonly BroadcastingContext _context;
    private readonly Mock<ILogger<UserPlatformTwitterSettingsDataStore>> _logger = new();
    private readonly UserPlatformTwitterSettingsDataStore _dataStore;

    public UserPlatformTwitterSettingsDataStoreTests()
    {
        ...
        _dataStore = new UserPlatformTwitterSettingsDataStore(
            _context,
    ...
    {
        _context.UserPlatformTwitterSettings.Add(new UserPlatformTwitterSettings
        {
    ...
    public async Task GetByIdAsync_ReturnsSettingsById()
    {
        ...
        var entity = await _context.UserPlatformTwitterSettings.FirstAsync(s => s.CreatedByEntraOid == ownerOid);
    ...
    public async Task SaveAsync_CreatesNewSettings()
    {
        // Arrange
        const string ownerOid = "user-a-oid-11111111";
        var newSettings = new Domain.Models.UserPlatformTwitterSettings
        {
            CreatedByEntraOid = ownerOid,
            IsEnabled = true,
            HasConsumerKey = true,
    ...
    public async Task SaveAsync_UpdatesExistingSettingsForSameOwner()
    {
        ...
        var updatedSettings = new Domain.Models.UserPlatformTwitterSettings
        {
            CreatedByEntraOid = ownerOid,
            IsEnabled = true,
            HasConsumerKey = true,
        ...
        var count = await _context.UserPlatformTwitterSettings
    ...
    public async Task DeleteAsync_DeletesWhenOwnerMatches()
    {
        ...
        // Assert
        Assert.True(result);
        Assert.Null(await _context.UserPlatformTwitterSettings.FindAsync(settingsId));
    }
    ...
    public async Task DeleteAsync_ReturnsFalseWhenOwnerMismatch()
    {
        ...
        var entity = await _context.UserPlatformTwitterSettings.FirstAsync(s => s.CreatedByEntraOid == ownerAOid);
        var settingsId = entity.Id;
        ...
        // Assert
        Assert.False(result);
        var stillExists = await _context.UserPlatformTwitterSettings.FindAsync(settingsId);
