...
public class EngagementDataStoreTests : IDisposable
{
    ...
    [Fact]
    public async Task GetAllAsync_WithDefaultSort_ReturnsEngagementsDescendingByStartDate()
    {
        // Arrange
        _context.Engagements.AddRange(
            CreateEngagement(name: "Conference A")
                .With(e => e.StartDateTime = new DateTimeOffset(2025, 3, 1, 9, 0, 0, TimeSpan.Zero)),
            CreateEngagement(name: "Conference B")
                .With(e => e.StartDateTime = new DateTimeOffset(2025, 5, 1, 9, 0, 0, TimeSpan.Zero)),
            CreateEngagement(name: "Conference C")
                .With(e => e.StartDateTime = new DateTimeOffset(2025, 1, 1, 9, 0, 0, TimeSpan.Zero))
        );
        await _context.SaveChangesAsync();
        ...
        Assert.Equal("Conference B", result.Items[0].Name);
        Assert.Equal("Conference A", result.Items[1].Name);
        Assert.Equal("Conference C", result.Items[2].Name);
    }

    [Fact]
    public async Task GetAllAsync_WithNameSortAscending_ReturnsEngagementsAscendingByName()
    {
        // Arrange
        _context.Engagements.AddRange(
            CreateEngagement(name: "Zebra Conference"),
            CreateEngagement(name: "Alpha Conference"),
            CreateEngagement(name: "Beta Conference")
        );
        await _context.SaveChangesAsync();

        // Act
        var result = await _dataStore.GetAllAsync(1, 10, "name", false);

        // Assert
        Assert.NotNull(result);
        Assert.Equal(3, result.Items.Count);
        Assert.Equal("Alpha Conference", result.Items[0].Name);
        Assert.Equal("Beta Conference", result.Items[1].Name);
        Assert.Equal("Zebra Conference", result.Items[2].Name);
    }

    [Fact]
    public async Task GetAllAsync_WithEndDateSortDescending_ReturnsEngagementsDescendingByEndDate()
    {
        // Arrange
        _context.Engagements.AddRange(
            CreateEngagement(name: "Conference A")
                .With(e => e.EndDateTime = new DateTimeOffset(2025, 3, 10, 17, 0, 0, TimeSpan.Zero)),
            CreateEngagement(name: "Conference B")
                .With(e => e.EndDateTime = new DateTimeOffset(2025, 6, 15, 17, 0, 0, TimeSpan.Zero)),
            CreateEngagement(name: "Conference C")
                .With(e => e.EndDateTime = new DateTimeOffset(2025, 1, 5, 17, 0, 0, TimeSpan.Zero))
        );
        await _context.SaveChangesAsync();

        // Act
        var result = await _dataStore.GetAllAsync(1, 10, "enddate", true);
    ...
    [Fact]
    public async Task GetAllAsync_WithFilterConference_ReturnsOnlyMatchingEngagements()
    {
        // Arrange
        _context.Engagements.AddRange(
            CreateEngagement(name: "Tech Conference 2025"),
            CreateEngagement(name: "Developer Summit"),
            CreateEngagement(name: "Code Conference")
        );
        await _context.SaveChangesAsync();

        // Act
        var result = await _dataStore.GetAllAsync(1, 10, "startdate", true, "conference");

        // Assert
        Assert.NotNull(result);
        Assert.Equal(2, result.TotalCount);
        Assert.Equal(2, result.Items.Count);
        Assert.Contains(result.Items, e => e.Name == "Tech Conference 2025");
        Assert.Contains(result.Items, e => e.Name == "Code Conference");
    }

    [Fact]
    public async Task GetAllAsync_WithNullFilter_ReturnsAllEngagements()
    {
        // Arrange
        _context.Engagements.AddRange(
            CreateEngagement(name: "Conference A"),
            CreateEngagement(name: "Summit B"),
            CreateEngagement(name: "Meetup C")
        );
        await _context.SaveChangesAsync();

        // Act
        var result = await _dataStore.GetAllAsync(1, 10, "startdate", true, null);
    ...
    [Fact]
    public async Task GetAllAsync_WithSortAndFilter_ReturnsFilteredAndSortedEngagements()
    {
        // Arrange
        _context.Engagements.AddRange(
            CreateEngagement(name: "Zebra Tech Conference")
                .With(e => e.StartDateTime = new DateTimeOffset(2025, 1, 1, 9, 0, 0, TimeSpan.Zero)),
            CreateEngagement(name: "Alpha Tech Conference")
                .With(e => e.StartDateTime = new DateTimeOffset(2025, 3, 1, 9, 0, 0, TimeSpan.Zero)),
            CreateEngagement(name: "Beta Developer Summit")
                .With(e => e.StartDateTime = new DateTimeOffset(2025, 2, 1, 9, 0, 0, TimeSpan.Zero))
        );
        await _context.SaveChangesAsync();

        // Act
        var result = await _dataStore.GetAllAsync(1, 10, "name", false, "conference");
...
file static class EngagementExtensions
{
    public static Models.Engagement With(this Models.Engagement engagement, Action<Models.Engagement> modifier)
    {
        modifier(engagement);
        return engagement;
    }
