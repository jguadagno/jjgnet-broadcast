...
/// <summary>
/// Tests for UserEventDistributorMappingDataStore.
/// </summary>
public class UserEventDistributorMappingDataStoreTests : IDisposable
{
    private readonly BroadcastingContext _context;
    private readonly Mock<ILogger<UserEventDistributorMappingDataStore>> _logger = new();
    private readonly UserEventDistributorMappingDataStore _dataStore;

    public UserEventDistributorMappingDataStoreTests()
    {
        ...
        _dataStore = new UserEventDistributorMappingDataStore(
            _context,
    ...
    private async Task CreateMappingAsync(
        ...
        bool isActive = true)
    {
        _context.UserEventDistributorMappings.Add(new UserEventDistributorMapping
        {
    ...
    public async Task GetByIdAsync_ReturnsMappingById()
    {
        ...
        var entity = await _context.UserEventDistributorMappings.FirstAsync(s => s.CreatedByEntraOid == ownerOid);
    ...
    {
        const string ownerOid = "user-a-oid-11111111";
        var mapping = new Domain.Models.UserEventDistributorMapping
        {
            CreatedByEntraOid = ownerOid,
        ...
        var persisted = await _context.UserEventDistributorMappings.FirstAsync(m => m.Id == result.Id);
    ...
    public async Task SaveAsync_UpdatesExistingMappingById()
    {
        ...
        var existing = await _context.UserEventDistributorMappings.FirstAsync(m => m.CreatedByEntraOid == ownerOid);
        ...
        var count = await _context.UserEventDistributorMappings.CountAsync(m => m.CreatedByEntraOid == ownerOid);
    ...
    public async Task DeleteAsync_ReturnsFalseWhenOwnerMismatch()
    {
        ...
        const string ownerBOid = "user-b-oid-22222222";
        await CreateMappingAsync(ownerAOid, Domain.Constants.MessageTemplates.MessageTypes.NewSyndicationFeedItem, 1);
        var entity = await _context.UserEventDistributorMappings.FirstAsync(m => m.CreatedByEntraOid == ownerAOid);

        var result = await _dataStore.DeleteAsync(entity.Id, ownerBOid);

        Assert.False(result);
        Assert.NotNull(await _context.UserEventDistributorMappings.FindAsync(entity.Id));
    }
}
