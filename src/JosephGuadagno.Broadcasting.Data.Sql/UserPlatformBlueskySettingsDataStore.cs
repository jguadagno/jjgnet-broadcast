...
/// </summary>
public class UserPlatformBlueskySettingsDataStore(
    BroadcastingContext broadcastingContext,
    IMapper mapper,
    ILogger<UserPlatformBlueskySettingsDataStore> logger) : IUserPlatformBlueskySettingsDataStore
{
    /// <inheritdoc />
    public async Task<UserPlatformBlueskySettings?> GetByUserAsync(
        string ownerOid, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            var entity = await broadcastingContext.UserPlatformBlueskySettings
                .AsNoTracking()
                .FirstOrDefaultAsync(s => s.CreatedByEntraOid == ownerOid, cancellationToken);

            return entity is null ? null : mapper.Map<UserPlatformBlueskySettings>(entity);
        }
        catch (Exception ex)
        {
            logger.LogError(
    ...
    /// <inheritdoc />
    public async Task<UserPlatformBlueskySettings?> GetByIdAsync(
    ...
    /// <inheritdoc />
    public async Task<UserPlatformBlueskySettings?> SaveAsync(
        UserPlatformBlueskySettings settings, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            var existing = await broadcastingContext.UserPlatformBlueskySettings
                .FirstOrDefaultAsync(s => s.CreatedByEntraOid == settings.CreatedByEntraOid, cancellationToken);

            if (existing is null)
            {
                existing = new Models.UserPlatformBlueskySettings
                {
                    CreatedByEntraOid = settings.CreatedByEntraOid,
                    CreatedOn = DateTimeOffset.UtcNow
                };
                broadcastingContext.UserPlatformBlueskySettings.Add(existing);
            }
            ...
            return mapper.Map<UserPlatformBlueskySettings>(existing);
        }
        catch (Exception ex)
        {
            logger.LogError(
        ...
        int id, string ownerOid, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            broadcastingContext.UserPlatformBlueskySettings.Remove(existing);
