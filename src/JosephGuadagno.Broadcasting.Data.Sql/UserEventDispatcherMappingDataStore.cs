...
/// </summary>
public class UserEventDispatcherMappingDataStore(
    BroadcastingContext broadcastingContext,
    IMapper mapper,
    ILogger<UserEventDispatcherMappingDataStore> logger) : IUserEventDispatcherMappingDataStore
{
    /// <inheritdoc />
    public async Task<List<UserEventDispatcherMapping>> GetByUserAsync(
        string ownerOid,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var query = broadcastingContext.UserEventDispatcherMappings
            .AsNoTracking()
        ...
        return mapper.Map<List<UserEventDispatcherMapping>>(entities);
    }

    /// <inheritdoc />
    public async Task<List<UserEventDispatcherMapping>> GetByUserAndEventTypeAsync(
        string ownerOid,
        string eventType,
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            var entities = await broadcastingContext.UserEventDispatcherMappings
                .AsNoTracking()
    ...
    /// <inheritdoc />
    public async Task<UserEventDispatcherMapping?> GetByIdAsync(
        int id,
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            var entity = await broadcastingContext.UserEventDispatcherMappings
                .AsNoTracking()
                .FirstOrDefaultAsync(m => m.Id == id, cancellationToken);

            return entity is null ? null : mapper.Map<UserEventDispatcherMapping>(entity);
        }
        catch (Exception ex)
        {
    ...
    /// <inheritdoc />
    public async Task<UserEventDispatcherMapping?> SaveAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            Models.UserEventDispatcherMapping? existing = null;
            if (mapping.Id > 0)
            {
                existing = await broadcastingContext.UserEventDispatcherMappings
                    .FirstOrDefaultAsync(
            ...
            if (existing is null)
            {
                existing = new Models.UserEventDispatcherMapping
                {
                    CreatedByEntraOid = mapping.CreatedByEntraOid,
                    CreatedOn = DateTimeOffset.UtcNow
                };
                broadcastingContext.UserEventDispatcherMappings.Add(existing);
            }
            ...
            return mapper.Map<UserEventDispatcherMapping>(existing);
        }
        catch (Exception ex)
        {
            logger.LogError(
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            var existing = await broadcastingContext.UserEventDispatcherMappings
                .FirstOrDefaultAsync(
            ...
            broadcastingContext.UserEventDispatcherMappings.Remove(existing);
