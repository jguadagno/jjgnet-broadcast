...
public class ScheduledItems(
    ...
    IScheduledItemEventDispatcher scheduledItemEventDispatcher,
    IFeedCheckManager feedCheckManager,
    ILogger<ScheduledItems> logger)
{
    [Function(ConfigurationFunctionNames.DispatchersScheduledItems)]
    public async Task RunAsync([TimerTrigger("%dispatchers_scheduled_items_cron_settings%")]
        TimerInfo myTimer)
    {
        var startedAt = DateTimeOffset.UtcNow;
        logger.LogDebug("{FunctionName} started at: {StartedAt:f}",
            ConfigurationFunctionNames.DispatchersScheduledItems, startedAt);
        ...
        foreach (var ownerGroup in itemsByOwner)
        {
            ...
            var feedCheck = await feedCheckManager.GetByNameAsync(
                ConfigurationFunctionNames.DispatchersScheduledItems, ownerOid
            ) ?? new FeedCheck
            {
