    ...
    ILogger<ProcessNewSyndicationDataFired> logger)
{
    ...
    public async Task<SocialMediaPublishRequest?> RunAsync([EventGridTrigger] EventGridEvent eventGridEvent)
    {
        ...
        if (template is null)
        {
	        logger.LogWarning("No template found for {Platform} / {MessageType} for owner {Id}", MessageTemplates.Platforms.Twitter, MessageTemplates.MessageTypes.NewSyndicationFeedItem, ownerEntraOid);
	        return null;
        }
