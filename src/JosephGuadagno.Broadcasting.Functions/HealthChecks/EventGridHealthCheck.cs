...
/// </summary>
internal sealed class EventGridHealthCheck(IEventPublisherSettings settings) : IHealthCheck
{
	public Task<HealthCheckResult> CheckHealthAsync(
        HealthCheckContext context,
        CancellationToken cancellationToken = default)
    {
        if (settings.TopicEndpointSettings is null || settings.TopicEndpointSettings.Count == 0)
        {
            return Task.FromResult(HealthCheckResult.Unhealthy(
                "No EventGrid topic endpoints are configured."));
        }

        var misconfigured = settings.TopicEndpointSettings
        ...
        var topicCount = settings.TopicEndpointSettings.Count;
