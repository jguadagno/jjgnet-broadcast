...
public class CollectorEventPublisher(
    IUserEventPublisherMappingDataStore userEventPublisherMappingDataStore,
    IMessageTemplateManager messageTemplateManager,
    IPostComposer postComposer,
    QueueServiceClient queueServiceClient,
    ILogger<CollectorEventPublisher> logger) : ICollectorEventPublisher
{
    ...
    public async Task PublishSyndicationFeedItemAsync(SyndicationFeedItem item, string ownerOid, CancellationToken cancellationToken = default)
    {
        var mappings = await userEventPublisherMappingDataStore.GetByUserAndEventTypeAsync(
            ownerOid, MessageTemplates.MessageTypes.NewSyndicationFeedItem, cancellationToken);

        if (mappings.Count == 0)
        {
            logger.LogDebug("No active publisher mappings for owner '{OwnerOid}' / {EventType}",
    ...
    public async Task PublishYouTubeItemAsync(YouTubeItem item, string ownerOid, CancellationToken cancellationToken = default)
    {
        var mappings = await userEventPublisherMappingDataStore.GetByUserAndEventTypeAsync(
    ...
    public async Task PublishSpeakingEngagementAsync(Engagement item, string ownerOid, CancellationToken cancellationToken = default)
    {
        var mappings = await userEventPublisherMappingDataStore.GetByUserAndEventTypeAsync(
        ...
        CancellationToken cancellationToken)
    {
        ...
        await queueClient.SendMessageAsync(JsonSerializer.Serialize(publishRequest));
