    ...
    ILogger<ProcessNewYouTubeDataFired> logger)
{
    ...
    public async Task<SocialMediaPublishRequest?> RunAsync([EventGridTrigger] EventGridEvent eventGridEvent)
    {
        ...
        if (template is null)
        {
	        logger.LogWarning("No template found for {Platform} / {MessageType} for owner {Id}", MessageTemplates.Platforms.Facebook, MessageTemplates.MessageTypes.NewYouTubeItem, ownerEntraOid);
	        return null;
        }
