...
{
    /// <summary>
    /// Gets all dispatcher settings (Bluesky, Twitter, LinkedIn, Facebook) for the resolved owner in one call.
    /// </summary>
    /// <param name="ownerOid">Optional Entra OID. Non-admin callers can only query their own settings.</param>
    /// <response code="200">Returns the aggregate dispatcher settings (null for unconfigured platforms).</response>
    /// <response code="401">Not authenticated.</response>
    /// <response code="403">Caller may not query the requested owner.</response>
    [HttpGet]
    [Authorize(Policy = AuthorizationPolicyNames.RequireViewer)]
    [ProducesResponseType(StatusCodes.Status200OK, Type = typeof(DispatchersAggregateResponse))]
    [ProducesResponseType(StatusCodes.Status401Unauthorized)]
    [ProducesResponseType(StatusCodes.Status403Forbidden)]
    public async Task<ActionResult<DispatchersAggregateResponse>> GetAllAsync([FromQuery] string? ownerOid = null)
    {
        ...
        return Ok(new DispatchersAggregateResponse
        {
