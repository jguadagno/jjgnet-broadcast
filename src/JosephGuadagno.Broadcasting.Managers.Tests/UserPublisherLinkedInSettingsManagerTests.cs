...
public class UserPublisherLinkedInSettingsManagerTests
{
    private readonly Mock<IUserPublisherLinkedInSettingsDataStore> _dataStore = new();
    private readonly Mock<IKeyVault> _keyVault = new();
    private readonly UserPublisherLinkedInSettingsManager _sut;

    public UserPublisherLinkedInSettingsManagerTests()
    {
        _sut = new UserPublisherLinkedInSettingsManager(
            _dataStore.Object,
            _keyVault.Object,
            NullLogger<UserPublisherLinkedInSettingsManager>.Instance);
    }
    ...
    public async Task GetAsync_DelegatesToDataStore()
    {
        ...
        var expected = new UserPublisherLinkedInSettings { CreatedByEntraOid = ownerOid, IsEnabled = true };
    ...
    {
        const string ownerOid = "owner-1";
        var settings = new UserPublisherLinkedInSettings { CreatedByEntraOid = ownerOid, IsEnabled = true };
    ...
    {
        const string ownerOid = "owner-1";
        var existing = new UserPublisherLinkedInSettings { Id = 42, CreatedByEntraOid = ownerOid };
        _dataStore.Setup(d => d.GetByUserAsync(ownerOid, It.IsAny<CancellationToken>()))
