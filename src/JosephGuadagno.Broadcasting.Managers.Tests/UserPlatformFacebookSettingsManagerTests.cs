...
public class UserPlatformFacebookSettingsManagerTests
{
    private readonly Mock<IUserPlatformFacebookSettingsDataStore> _dataStore = new();
    private readonly Mock<IKeyVault> _keyVault = new();
    private readonly UserPlatformFacebookSettingsManager _sut;

    public UserPlatformFacebookSettingsManagerTests()
    {
        _sut = new UserPlatformFacebookSettingsManager(
            _dataStore.Object,
            _keyVault.Object,
            NullLogger<UserPlatformFacebookSettingsManager>.Instance);
    }
    ...
    public async Task GetAsync_DelegatesToDataStore()
    {
        ...
        var expected = new UserPlatformFacebookSettings { CreatedByEntraOid = ownerOid, IsEnabled = true };
    ...
    {
        const string ownerOid = "owner-1";
        var settings = new UserPlatformFacebookSettings { CreatedByEntraOid = ownerOid, IsEnabled = true };
    ...
    {
        const string ownerOid = "owner-1";
        var existing = new UserPlatformFacebookSettings { Id = 42, CreatedByEntraOid = ownerOid };
        _dataStore.Setup(d => d.GetByUserAsync(ownerOid, It.IsAny<CancellationToken>()))
