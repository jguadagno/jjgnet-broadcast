...
public class UserPlatformBlueskySettingsManagerTests
{
    private readonly Mock<IUserPlatformBlueskySettingsDataStore> _dataStore = new();
    private readonly Mock<IKeyVault> _keyVault = new();
    private readonly UserPlatformBlueskySettingsManager _sut;

    public UserPlatformBlueskySettingsManagerTests()
    {
        _sut = new UserPlatformBlueskySettingsManager(
            _dataStore.Object,
            _keyVault.Object,
            NullLogger<UserPlatformBlueskySettingsManager>.Instance);
    }
    ...
    public async Task GetAsync_DelegatesToDataStore()
    {
        ...
        var expected = new UserPlatformBlueskySettings { CreatedByEntraOid = ownerOid, IsEnabled = true };
    ...
    {
        const string ownerOid = "owner-1";
        var settings = new UserPlatformBlueskySettings { CreatedByEntraOid = ownerOid, IsEnabled = true };
    ...
    {
        const string ownerOid = "owner-1";
        var existing = new UserPlatformBlueskySettings { Id = 42, CreatedByEntraOid = ownerOid };
        _dataStore.Setup(d => d.GetByUserAsync(ownerOid, It.IsAny<CancellationToken>()))
