...
public class UserPublisherBlueskySettingsManagerTests
{
    private readonly Mock<IUserPublisherBlueskySettingsDataStore> _dataStore = new();
    private readonly Mock<IKeyVault> _keyVault = new();
    private readonly UserPublisherBlueskySettingsManager _sut;

    public UserPublisherBlueskySettingsManagerTests()
    {
        _sut = new UserPublisherBlueskySettingsManager(
            _dataStore.Object,
            _keyVault.Object,
            NullLogger<UserPublisherBlueskySettingsManager>.Instance);
    }
    ...
    public async Task GetAsync_DelegatesToDataStore()
    {
        ...
        var expected = new UserPublisherBlueskySettings { CreatedByEntraOid = ownerOid, IsEnabled = true };
    ...
    {
        const string ownerOid = "owner-1";
        var settings = new UserPublisherBlueskySettings { CreatedByEntraOid = ownerOid, IsEnabled = true };
    ...
    {
        const string ownerOid = "owner-1";
        var existing = new UserPublisherBlueskySettings { Id = 42, CreatedByEntraOid = ownerOid };
        _dataStore.Setup(d => d.GetByUserAsync(ownerOid, It.IsAny<CancellationToken>()))
    ...
    [Theory]
    [InlineData("owner-1", "publisher-owner-1-bluesky-app-password")]
    [InlineData("owner@with#special!", "publisher-owner-with-special--bluesky-app-password")]
    public async Task StoreAppPasswordAsync_BuildsCorrectSecretName(string ownerOid, string expectedSecretName)
    {
        ...
        await _sut.StoreAppPasswordAsync(ownerOid, "my-app-password");
