...
public class UserPublisherTwitterSettingsManagerTests
{
    private readonly Mock<IUserPublisherTwitterSettingsDataStore> _dataStore = new();
    private readonly Mock<IKeyVault> _keyVault = new();
    private readonly UserPublisherTwitterSettingsManager _sut;

    public UserPublisherTwitterSettingsManagerTests()
    {
        _sut = new UserPublisherTwitterSettingsManager(
            _dataStore.Object,
            _keyVault.Object,
            NullLogger<UserPublisherTwitterSettingsManager>.Instance);
    }
    ...
    public async Task GetAsync_DelegatesToDataStore()
    {
        ...
        var expected = new UserPublisherTwitterSettings { CreatedByEntraOid = ownerOid, IsEnabled = true };
    ...
    public async Task GetAsync_ReturnsNullWhenNoSettings()
    {
        _dataStore.Setup(d => d.GetByUserAsync(It.IsAny<string>(), It.IsAny<CancellationToken>()))
            .ReturnsAsync((UserPublisherTwitterSettings?)null);
    ...
    {
        const string ownerOid = "owner-1";
        var settings = new UserPublisherTwitterSettings { CreatedByEntraOid = ownerOid, IsEnabled = true };
    ...
    {
        const string ownerOid = "owner-1";
        var existing = new UserPublisherTwitterSettings { Id = 42, CreatedByEntraOid = ownerOid };
        _dataStore.Setup(d => d.GetByUserAsync(ownerOid, It.IsAny<CancellationToken>()))
