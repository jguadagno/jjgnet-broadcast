...
/// <summary>
/// Data store for managing per-user RSS/Atom/JSON feed collector configurations
/// </summary>
public interface IUserCollectorFeedSourceDataStore
{
    /// <summary>
    /// Retrieves feed source configurations for a specific user
    /// </summary>
    ...
    Task<List<UserCollectorFeedSource>> GetByUserAsync(string ownerOid, bool activeOnly = false, CancellationToken cancellationToken = default);
    ...
    Task<PagedResult<UserCollectorFeedSource>> GetAllAsync(string ownerOid, int page, int pageSize, string sortBy = "displayname", bool sortDescending = false, string? filter = null, CancellationToken cancellationToken = default);
}
