...
/// </summary>
public class UserPlatformTwitterSettingsManager(
	IUserPlatformTwitterSettingsDataStore UserPlatformTwitterSettingsDataStore,
	IKeyVault keyVault,
	ILogger<UserPlatformTwitterSettingsManager> logger)
	: IUserPlatformTwitterSettingsManager
{
	/// <inheritdoc />
    public Task<UserPlatformTwitterSettings?> GetAsync(string ownerOid, CancellationToken cancellationToken = default)
    {
        ArgumentException.ThrowIfNullOrWhiteSpace(ownerOid);
        return UserPlatformTwitterSettingsDataStore.GetByUserAsync(ownerOid, cancellationToken);
    }

    /// <inheritdoc />
    public Task<UserPlatformTwitterSettings?> SaveAsync(UserPlatformTwitterSettings settings, CancellationToken cancellationToken = default)
    {
        ArgumentNullException.ThrowIfNull(settings);
        ArgumentException.ThrowIfNullOrWhiteSpace(settings.CreatedByEntraOid);
        return UserPlatformTwitterSettingsDataStore.SaveAsync(settings, cancellationToken);
    }
    ...
    {
        ArgumentException.ThrowIfNullOrWhiteSpace(ownerOid);
        var existing = await UserPlatformTwitterSettingsDataStore.GetByUserAsync(ownerOid, cancellationToken);
        if (existing is null)
        {
            return false;
        }
        return await UserPlatformTwitterSettingsDataStore.DeleteAsync(existing.Id, ownerOid, cancellationToken);
    }
