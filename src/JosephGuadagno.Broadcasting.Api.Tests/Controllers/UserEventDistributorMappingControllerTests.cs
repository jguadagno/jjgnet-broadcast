...
public class UserEventDistributorMappingControllerTests
{
    private readonly Mock<IUserEventDistributorMappingManager> _managerMock = new();
    private readonly Mock<IOnboardingManager> _onboardingManagerMock = new();
    private readonly Mock<ILogger<UserEventDistributorMappingController>> _loggerMock = new();

    private static readonly IMapper Mapper = ApiTestMapper.Instance;

    private UserEventDistributorMappingController CreateSut(string ownerOid = "owner-oid-12345", bool isSiteAdmin = false)
    {
        return new UserEventDistributorMappingController(
            _managerMock.Object,
    ...
    private static UserEventDistributorMapping BuildMapping(int id = 1, string ownerOid = "owner-oid-12345") => new()
    {
        Id = id,
    ...
    {
        var items = new List<UserEventDistributorMapping>
        {
            BuildMapping(1),
            BuildMapping(2)
        };
        ...
        var response = okResult.Value.Should().BeAssignableTo<List<UserEventDistributorMappingResponse>>().Subject;
    ...
    {
        _managerMock.Setup(m => m.GetByIdAsync(8, It.IsAny<CancellationToken>())).ReturnsAsync((UserEventDistributorMapping?)null);
    ...
    public async Task CreateAsync_WhenModelStateIsInvalid_ReturnsBadRequest()
    {
        ...
        var result = await sut.CreateAsync(new CreateUserEventDistributorMappingRequest());

        result.Result.Should().BeOfType<BadRequestObjectResult>();
        _managerMock.Verify(m => m.SaveAsync(It.IsAny<UserEventDistributorMapping>(), It.IsAny<CancellationToken>()), Times.Never);
    }

    [Fact]
    public async Task CreateAsync_WhenSaveSucceeds_ReturnsCreatedAtAction()
    {
        var request = new CreateUserEventDistributorMappingRequest
        {
        ...
        UserEventDistributorMapping? captured = null;
        _managerMock.Setup(m => m.SaveAsync(It.IsAny<UserEventDistributorMapping>(), It.IsAny<CancellationToken>()))
            .Callback<UserEventDistributorMapping, CancellationToken>((mapping, _) => captured = mapping)
            .ReturnsAsync(saved);
        ...
        createdResult.ActionName.Should().Be(nameof(UserEventDistributorMappingController.GetAsync));
    ...
    {
        _managerMock.Setup(m => m.GetByIdAsync(11, It.IsAny<CancellationToken>())).ReturnsAsync((UserEventDistributorMapping?)null);

        var sut = CreateSut();

        var result = await sut.UpdateAsync(11, new UpdateUserEventDistributorMappingRequest());
    ...
    public async Task UpdateAsync_WhenItemBelongsToAnotherUser_ReturnsForbid()
    {
        ...
        var result = await sut.UpdateAsync(12, new UpdateUserEventDistributorMappingRequest { IsActive = false });
    ...
    public async Task UpdateAsync_WhenOptionalFieldsAreOmitted_KeepsExistingValues()
    {
        ...
        var result = await sut.UpdateAsync(13, new UpdateUserEventDistributorMappingRequest { IsActive = false });
    ...
    {
        _managerMock.Setup(m => m.GetByIdAsync(14, It.IsAny<CancellationToken>())).ReturnsAsync((UserEventDistributorMapping?)null);
