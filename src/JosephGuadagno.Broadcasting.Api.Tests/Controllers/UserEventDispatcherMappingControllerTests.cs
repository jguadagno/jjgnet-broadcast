...
public class UserEventDispatcherMappingControllerTests
{
    private readonly Mock<IUserEventDispatcherMappingManager> _managerMock = new();
    private readonly Mock<IOnboardingManager> _onboardingManagerMock = new();
    private readonly Mock<ILogger<UserEventDispatcherMappingController>> _loggerMock = new();

    private static readonly IMapper Mapper = ApiTestMapper.Instance;

    private UserEventDispatcherMappingController CreateSut(string ownerOid = "owner-oid-12345", bool isSiteAdmin = false)
    {
        return new UserEventDispatcherMappingController(
            _managerMock.Object,
    ...
    private static UserEventDispatcherMapping BuildMapping(int id = 1, string ownerOid = "owner-oid-12345") => new()
    {
        Id = id,
    ...
    {
        var items = new List<UserEventDispatcherMapping>
        {
            BuildMapping(1),
            BuildMapping(2)
        };
        ...
        var response = okResult.Value.Should().BeAssignableTo<List<UserEventDispatcherMappingResponse>>().Subject;
    ...
    {
        _managerMock.Setup(m => m.GetByIdAsync(8, It.IsAny<CancellationToken>())).ReturnsAsync((UserEventDispatcherMapping?)null);
    ...
    public async Task CreateAsync_WhenModelStateIsInvalid_ReturnsBadRequest()
    {
        ...
        var result = await sut.CreateAsync(new CreateUserEventDispatcherMappingRequest());

        result.Result.Should().BeOfType<BadRequestObjectResult>();
        _managerMock.Verify(m => m.SaveAsync(It.IsAny<UserEventDispatcherMapping>(), It.IsAny<CancellationToken>()), Times.Never);
    }

    [Fact]
    public async Task CreateAsync_WhenSaveSucceeds_ReturnsCreatedAtAction()
    {
        var request = new CreateUserEventDispatcherMappingRequest
        {
        ...
        UserEventDispatcherMapping? captured = null;
        _managerMock.Setup(m => m.SaveAsync(It.IsAny<UserEventDispatcherMapping>(), It.IsAny<CancellationToken>()))
            .Callback<UserEventDispatcherMapping, CancellationToken>((mapping, _) => captured = mapping)
            .ReturnsAsync(saved);
        ...
        createdResult.ActionName.Should().Be(nameof(UserEventDispatcherMappingController.GetAsync));
    ...
    {
        _managerMock.Setup(m => m.GetByIdAsync(11, It.IsAny<CancellationToken>())).ReturnsAsync((UserEventDispatcherMapping?)null);

        var sut = CreateSut();

        var result = await sut.UpdateAsync(11, new UpdateUserEventDispatcherMappingRequest());
    ...
    public async Task UpdateAsync_WhenItemBelongsToAnotherUser_ReturnsForbid()
    {
        ...
        var result = await sut.UpdateAsync(12, new UpdateUserEventDispatcherMappingRequest { IsActive = false });
    ...
    public async Task UpdateAsync_WhenOptionalFieldsAreOmitted_KeepsExistingValues()
    {
        ...
        var result = await sut.UpdateAsync(13, new UpdateUserEventDispatcherMappingRequest { IsActive = false });
    ...
    {
        _managerMock.Setup(m => m.GetByIdAsync(14, It.IsAny<CancellationToken>())).ReturnsAsync((UserEventDispatcherMapping?)null);
