...
# Tank QA Report — Test Failures on `issue-980-publisher-architecture-refactor`

**Date:** 2026-05-21  
**Branch:** `issue-980-publisher-architecture-refactor`  
**Reported by:** Tank (Tester)
...
Both failures are **directly caused by uncommitted changes on this branch** — NOT pre-existing.
...
- **File:** `src/JosephGuadagno.Broadcasting.Functions.Tests/Collectors/LoadAllSpeakingEngagementsTests.cs:332`
- **Error:** `Assert.IsType() Failure — Expected OkObjectResult, Actual BadRequestObjectResult`
- **Root Cause:** Uncommitted change to `LoadAllSpeakingEngagements.cs` removed the null guard:
...
When the reader returns `null`, `null.Count` throws `NullReferenceException`. The outer `catch` block catches it and returns `BadRequestObjectResult(exception.Message)`. The test expects `OkObjectResult` (treat null as zero items = graceful no-op).

### 2. `LoadNewPostsTests.RunAsync_HandlesNullFeedList_Gracefully`

- **File:** `src/JosephGuadagno.Broadcasting.Functions.Tests/Collectors/LoadNewPostsTests.cs:347`
- **Error:** `Assert.IsType() Failure — Expected OkObjectResult, Actual BadRequestObjectResult`
- **Root Cause:** Same pattern — uncommitted change to `LoadNewPosts.cs` removed the null guard:
...
Same NullReferenceException → catch → `BadRequestObjectResult`.
...
- `git diff HEAD -- <file>` confirms the `null ||` removal is an **uncommitted change** on the working tree.
- Both null guards were introduced in commit `4b765f88` (Sanity Check #957) alongside the `RunAsync_HandlesNull*_Gracefully` tests — they were always consistent in git.
- The branch's uncommitted work broke that consistency.
...
**File 1:** `src/JosephGuadagno.Broadcasting.Functions/Collectors/SpeakingEngagement/LoadAllSpeakingEngagements.cs`  
Restore line 67 to:
```csharp
if (newItems == null || newItems.Count == 0)
```

**File 2:** `src/JosephGuadagno.Broadcasting.Functions/Collectors/SyndicationFeed/LoadNewPosts.cs`  
Restore line 74 to:
```csharp
if (newItems == null || newItems.Count == 0)
```

**Note:** `LoadNewPosts.cs` also has two other uncommitted changes that are likely intentional:
1. Removed `using JosephGuadagno.Broadcasting.Functions.Interfaces;` — probably intentional cleanup
2. `syndicationFeedItemManager.SaveAsync(item)` → `syndicationFeedItemManager.SaveAsync(item, ct)` — passes CancellationToken correctly
3. Removed a `continue;` in the catch block — probably intentional (no effect, catch is the last statement in the loop body)

Only the `null ||` removal is the defect.
