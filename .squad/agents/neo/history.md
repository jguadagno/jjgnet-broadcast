...
- PR #800 (2026-04-21): Shared `EntraClaimsTransformation` extraction for API Phase 0 foundation

**Auth architecture:** Web layer uses battle-tested `EntraClaimsTransformation` (SQL-backed, app-managed roles); API will migrate from 42 custom Entra scopes to role-based policies matching Web pattern. Scope-to-role migration phases (#763–#769) scheduled across future sprints to prevent blocking. **Phase 0 design pattern (intentionally additive):** API gains host-level auth infrastructure (claims transformation, role policies) without changing controller-level authorization behavior; Phase 1-4 migrates controllers from scope to role checks.

**Sprint 21 scope:** Collector owner OID completeness (Trinity #760/#761, Tank regression #762). Blocker: `scripts\database\data-seed.sql` requires CreatedByEntraOid bootstrap values; PR #771 stacked on #770; PR #772 has unrelated Web config drift. **Stacked PR policy:** each PR must validate against its current base; downstream PR cannot fix upstream break; after upstream merge, retarget and revalidate downstream before clearing.

**Sprint 22 Phase 0 API RBAC:** PR #801 — Issue #764. Restored to include API claims transformation, role policies, scope-based controller checks. **Blocked after re-review:** reviewer lockout (revision by Trinity after Neo rejection) + branch stale behind main (PR #800 merged). Requires reassignment to non-Trinity agent, rebase, history sync, rerun checks.

**PR review standards (Neo established):**
- Two-mode comment structure: Formal Review (comprehensive, audit-trail quality, for multi-finding PRs with verdict) / Quick Finding (minimal, targeted, for single blockers)
- One-PR-per-issue rule: blocking defect if PR carries unrelated payload (e.g., Web config drift in Functions PR)
- Stacked PR review gate: each PR must build and test against its current base
- Reviewer lockout: previously rejected artifact must be revised by a different agent before next review cycle
- Branch naming: `issue-<number>-<slug>` or `feature/<number>-<slug>` (issue number machine-readable)
- PR title format: `<type>(#<issue>): <summary>` with exactly one matching closing issue reference

**Settings design pattern (per #731 analysis):** Persistence layer stores flat key/value; business logic validates and builds typed objects; API layer masks secrets in responses. Enables write-only secret masking, partial updates, platform extensibility without migrations.

**Testing patterns:**
- New settings ownership tests model: per-user isolation with OID claim matching (PR #731/#732)
- Security test count target: 20+ tests for ownership enforcement (PR #739 precedent)
- Sealed types in tests: use typed null assertions

---

## 2026-04-21 — PR #801 Re-Review: Process Gates Remain Open

**Status:** 🔴 BLOCKED

### Summary

Neo re-reviewed PR #801 (API RBAC Phase 0 restoration). The technical implementation is sound:
- API registers shared `EntraClaimsTransformation`
- Mirrors Web role-policy hierarchy
- Preserves scope-based controller checks for dual enforcement
- GitHub checks: ✅ Green
- Local CI-aligned validation: ✅ Pass

Two process gates remain open:

### Blocking Findings

1. **Reviewer Lockout Violation (Hard Gate)**
   - PR #801 was previously rejected by Neo
   - New revision produced by Trinity (same author as rejection)
   - Squad policy: rejected artifacts must be revised by a *different* agent
   - Neo cannot clear until reassignment occurs

2. **Branch Staleness Against main**
   - PR #800 merged shared-claims extraction to main
   - PR #801 head `68c75aa` still behind main
   - Visible diff carries already-merged Phase 0 dependency files
   - `.squad/` history drift requires reconciliation
   - Branch must be refreshed, history resolved, checks rerun on updated head

### Required Actions

1. **Coordinator reassigns revision** to non-Trinity agent (options: Switch, Link, Morpheus, Oracle, etc.)
2. **Agent rebases** PR #801 on current main
3. **Agent resolves** `.squad/` history drift conflicts
4. **Agent reruns** validation (local + CI)
5. **PR returns to Neo** for clearance on current main

### Outcome

PR #801 is **BLOCKED** until process compliance gates close. Formal PR comment posted on #801.

**Related:** decisions.md entry 2026-04-21; orchestration log 2026-04-21T16-29-34Z
...
### 2026-04-21 — PR #801 Re-Review Gate
- Phase 0 technical acceptance is now satisfied when the API host wires `AddBroadcastingApiAuthorization()`, keeps controller `VerifyUserHasAnyAcceptedScope(...)` checks intact, and proves the shared claims transformation/policy chain with infrastructure tests.
- A previously rejected artifact cannot come back from the same author; reviewer lockout is a hard gate, so PR #801 must be revised by someone other than Trinity before it can be cleared.
- After upstream Phase 0 dependency PRs merge, the downstream PR must be refreshed against `main` so the visible diff drops already-merged files like `src\JosephGuadagno.Broadcasting.Managers\EntraClaimsTransformation.cs` and revalidation runs on the current base.
