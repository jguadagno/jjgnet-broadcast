...
# Random posts distributor

RandomPosts is a timer-driven distributor that works directly from per-user random-post settings instead of UserEventDistributorMappings. For each due row, it selects one syndication item, renders a RandomPost template with PostComposer, enqueues the message, and advances NextRunDateUtc.

## Flow

```mermaid
flowchart TD
    A[Timer trigger DistributorsRandomPosts] --> B[Load due UserRandomPostSettings]
...
- [`RandomPosts`](../../src/JosephGuadagno.Broadcasting.Functions/Distributors/RandomPosts.cs)
...
- [`RandomPosts.cs`](../../src/JosephGuadagno.Broadcasting.Functions/Distributors/RandomPosts.cs)
